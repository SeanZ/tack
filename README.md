...
问题不在 agent 不够强,而在这些**会话本身是易失的**,你缺一层把它们收成任务的东西。`pops` 只做这一件事:把易失的会话收束到持久的任务上。它是一个本地 CLI,数据是一个 jsonl 文件加几个 markdown,没有数据库,没有后台服务。
...
- **会话可以丢,任务不能丢。** AI 编码会话开得快关得快、上下文随窗口滚动而丢;你真正在推进的是任务,任务跨越多个会话、多个仓库、有时多个 agent。pops 把绑定这一层补上。
...
| 工具与数据分离 | 代码可分享,数据(`~/.pops`)私有且独立版本管理,互不牵连 |
...
3. **装依赖**:`bun install`。
4. **建 wrapper**:把 `~/.local/bin/pops` 写成 `#!/bin/bash` + `exec bun <仓库绝对路径>/bin/pops.ts "$@"`,`chmod +x`。若 `~/.local/bin` 不在 PATH,提示用户加入 rc,不要自动改 rc。
...
```
工具仓（代码，可分享）              ~/.pops（你的数据，私有）
├── bin/pops.ts   CLI 入口          ├── config.json   data_dir / repos / 默认值
├── web/          看板              └── data/         ← 独立 git 仓
├── hooks/        CC 钩子               ├── tasks.jsonl   一行一个 task
├── skills/pops/  通用 skill            └── tasks/<id>/digests/   会话摘要
└── config.example.json
```

路径解析:资产相对 checkout 解析;数据按 `POPS_DATA_DIR` → `config.json:data_dir` → `~/.pops/data` 解析。设 `POPS_HOME` / `POPS_DATA_DIR` 可跑隔离实例(测试用)。
...
pops tag <id> [tags...]         # 无参列出；untag 删
pops note <id> "<text>"         # 记进展（在会话中会自动关联当前会话）
pops set <id> key=val ...       # repo= target= state= ref= handoff_path= title=
pops which <uuid>               # 某会话属于哪个任务
pops scan [--repo r]            # 还没关联到任务的 CC 会话
pops web [--port 7420]          # 本地看板（前台运行，Ctrl+C 停）
pops setup cc                   # 安装 CC 会话钩子
pops config [key] [value]       # 读写配置
```

`pops task <verb> ...` 是 `pops <verb> ...` 的别名,两种写法等价。

## 会话怎么自动绑到任务

两条路径,都不需要你主动关闭会话:

- **主路径(边干边记)**:在 CC 会话里 `pops note <id> "..."` 记一条进展时,pops 读 `CLAUDE_CODE_SESSION_ID` 把**当前会话**自动绑到该任务。绑过之后,resume 这个会话时 SessionStart 钩子会把任务当前 brief 反喂给它,一上来就懂上下文。
- **备路径(事后收编)**:有些会话是随手裸开的,`pops scan` 能扫出未关联的会话,按内容判断该归到哪个任务或新建。

会话摘要(digest)在三个时机自动生成,固化进可备份的数据仓:`pops show` 时缺则惰性现算、归档(`mv archived`)时收尾固化、会话正常结束时 best-effort 兜底。CC 的 transcript 关闭后不删,所以绑过的会话永远可反查、可重新摘要。

## 本地看板

`pops web` 起一个本地网页:看任务在各状态/各仓库/各主线的分布,点开任意任务看它的全部进度记录和关联会话。看板只读,所有增删改仍走 CLI 或 AI。

## CC 钩子

`pops setup cc` 往 `~/.claude/settings.json` 装两个钩子:
